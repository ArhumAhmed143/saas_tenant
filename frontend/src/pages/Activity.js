...
export default function Dashboard() {
  ...
  const { projects, tasks, users, activities, currentUser, getOverdueTasks, getTeamStats } = useApp();
  ...
  // 🆕 BURNDOWN MOCK DATA (Last 5 days)
  const getBurndownData = () => {
    ...
    for (let i = 4; i >= 0; i--) {
      ...
      // Mock: Assume tasks completed linearly
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#f1f5f9' }}>
      ...
      <main style={{ marginLeft: 240, padding: 32, width: '100%' }}>
        ...
        {/* Progress Bar + Burndown + Quick Actions Row */}
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gap: 24, alignItems: 'start' }}>
          ...
          {/* 🆕 BURNDOWN CHART */}
          <div style={{ background: 'white', padding: 24, borderRadius: 16, border: '1px solid #e2e8f0' }}>
            <h3 style={{ fontSize: '1.1rem', marginBottom: 12, color: '#0f172a' }}>📉 Burndown (Mock)</h3>
